...
export function resolveNamespace (path, store) {
  ...
  for (let i = 0; i < fragments.length; i++) {
    ...
    if (fragment === '*') {
      ...
      const selected = store.state[parent].selected

      // TODO: Might want to check for `instanceof Entity` instead
      if (typeof selected === 'object') {
        resolved = resolved.replace('*', selected.id)
      } else {
        // TODO: Consider supporting a `strict` mode
        throw Error(`Failed to resolve namespace, no selected ${entity}`)
        // TODO: Consider falling back to `current` getter instead, probably the better option (could also make this optional)
      }
    }
  }
